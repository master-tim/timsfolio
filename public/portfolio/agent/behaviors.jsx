...
window.TD_AGENT = (() => {
  ...
  const handlers = {
    ...
    async listWork(push) {
      await push({
        type: "thought",
        text: "listing 6 of 18 projects, newest first.",
      });
      await sleep(350);
      await push({
        type: "tool",
        name: "list_work(limit=6)",
        status: "ok",
        result: "WORK_TABLE",
      });
    },
    ...
    async rag(push) {
      await projectCard(push, "rag-15k");
    },
    async aiGames(push) {
      await projectCard(push, "ai-games");
    },
    async cache(push) {
      await projectCard(push, "semantic-cache");
    },
    async engine(push) {
      await projectCard(push, "engine");
    },
    async robot(push) {
      await projectCard(push, "robot-face");
    },
    async fitts(push) {
      await projectCard(push, "fitts");
    },
    ...
    async skills(push) {
      await push({
        type: "assistant",
        text: `Roughly, in order of depth:
...
Not the person who'll build your fourteenth demo. The person who makes demo #1 a product.`,
    ...
    async weakness(push) {
      await push({
        type: "thought",
        text: "these are real answers, not interview answers.",
      });
      await sleep(400);
      await push({
        type: "assistant",
        text: `Three things I'm working on:
...
Counter-weight: none of these have cost anyone money. Yet.`,
    ...
    async ping(push) {
      ...
      await push({
        ...
        name: "ping()",
        status: "ok",
        result: { status: "alive", latency_ms: t, model: "td-agent@0.4.2" },
      });
    },
  ...
  async function respond(q, ctx) {
    ...
    try {
      await ctx.push({
        type: "thought",
        text: "no pattern match — routing to claude-haiku.",
      });
      await sleep(150);
      const res = await fetch("/api/agent", {
        ...
        body: JSON.stringify({
          query: q,
          tone: ctx.tone || "playful, dry, confident, concise",
        }),
      });
      if (res.ok) {
    ...
    await ctx.push({
      type: "assistant",
      text: `hmm, not sure how to route "${q}". try \`help\` or one of: tour · show work · why hire you · contact.`,
    });
  }
